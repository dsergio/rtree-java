 ...
 *
 */
public abstract class HyperRectangleBaseGeneric<T extends IRType<T>> implements IHyperRectangleGeneric<T> {
	...
	public HyperRectangleBaseGeneric(int numberDimensions) {
	...
	public boolean containsPoint(ILocationItemGeneric<T> item) {
	...
	public static <T extends IRType<T>> IHyperRectangleGeneric<T> sumRectanglesNDimensional(IHyperRectangleGeneric<T> rectangle, ILocationItemGeneric<T> item) {
		if (rectangle == null) {
			return null;
		}
		if (item == null) {
		...
		IHyperRectangleGeneric<T> newRect = new RectangleNDGeneric<T>(item.getNumberDimensions());
		for (int i = 0; i < item.getNumberDimensions(); i++) {
	...
	public static <T extends IRType<T>> boolean rectanglesOverlapNDimensional(IHyperRectangleGeneric<T> r1, IHyperRectangleGeneric<T> r2) {
	...
	public static <T extends IRType<T>> IHyperRectangleGeneric<T> twoPointsRectanglesNDimensional(ILocationItemGeneric<T> item1, ILocationItemGeneric<T> item2) {
		...
		IHyperRectangleGeneric<T> newRect = new RectangleNDGeneric<T>(item1.getNumberDimensions());
		for (int i = 0; i < item1.getNumberDimensions(); i++) {
			newRect.setDim1(i, min.get(i));
			newRect.setDim2(i, max.get(i));
		}
	...
	public static <T extends IRType<T>> double areaRectanglesNDimensional(IHyperRectangleGeneric<T> r1, IHyperRectangleGeneric<T> r2) {
	...
	public static <T extends IRType<T>> IHyperRectangleGeneric<T> sumRectanglesNDimensional(List<IHyperRectangleGeneric<T>> rectangles) {
		...
		if (rectangles.size() > 1) {
			...
			IHyperRectangleGeneric<T> rect = rectangles.get(0);
			for (int i = 0; i < rect.getNumberDimensions(); i++) {
				min.add(rect.getDim1(i));
				max.add(rect.getDim2(i));
			}
			
			for (IHyperRectangleGeneric<T> r : rectangles) {
				for (int i = 0; i < rect.getNumberDimensions(); i++) {
			...
			IHyperRectangleGeneric<T> newRect = new RectangleNDGeneric<T>(rect.getNumberDimensions());
			for (int i = 0; i < rect.getNumberDimensions(); i++) {
				newRect.setDim1(i, min.get(i));
				newRect.setDim2(i, max.get(i));
			}
