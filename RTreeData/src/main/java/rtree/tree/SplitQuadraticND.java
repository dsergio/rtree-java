...
public class SplitQuadraticND extends SplitBehaviorBase {
	
	final String description;
	
	public SplitQuadraticND() {
	...
	public void splitLeafNode(IRTreeNode node, ILocationItem locationItem) {
		...
		IRTreeNode node1 = new RTreeNodeND(node1Id, null, null, cache, logger);
		IRTreeNode node2 = new RTreeNodeND(node2Id, null, null, cache, logger);
...
//	        JSONArray childrenArr = root.getChildrenJSON();
	...
	public void splitBranchNode(IRTreeNode node) {
		...
		if (nodesChildren != null && nodesChildren.size() > maxChildren) {
			...
			for (int i = 0; i < nodesChildren.size(); i++) {
				if (i != index1 && i != index2) {
					if (HyperRectangleBase.areaRectanglesNDimensional(cache.getNode(nodesChildren.get(index1)).getRectangle(), cache.getNode(nodesChildren.get(i)).getRectangle()) <  HyperRectangleBase.areaRectanglesNDimensional(cache.getNode(nodesChildren.get(index2)).getRectangle(), cache.getNode(nodesChildren.get(i)).getRectangle())) {
			...
			IRTreeNode nodesParentsNewChild1 = new RTreeNodeND(nodesParentsNewChild1Id, null, null, cache, logger);
			IRTreeNode nodesParentsNewChild2 = new RTreeNodeND(nodesParentsNewChild2Id, null, null, cache, logger);
